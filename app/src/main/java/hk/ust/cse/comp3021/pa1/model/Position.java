...
public record Position(int row, int col) {
    ...
    public Position offsetBy(final int dRow, final int dCol) {
        // TODO done
        int rowCoor = this.row() + dRow;
        int colCoor = this.col() + dCol;
        if(rowCoor < 0 || colCoor < 0){
            throw new IllegalArgumentException("The resulting coordinate is invalid!");
        }
        return new Position(rowCoor, colCoor);
    }
    ...
    public Position offsetBy(@NotNull final PositionOffset offset) {
        // TODO done
        int rowCoor = this.row() + offset.dRow();
        int colCoor = this.col() + offset.dCol();
        if(rowCoor < 0 || colCoor < 0){
            throw new IllegalArgumentException("The resulting coordinate is invalid!");
        }
        return new Position(rowCoor, colCoor);
    ...
    public Position offsetByOrNull(final int dRow, final int dCol, final int numRows, final int numCols) {
        ...
        int rowCoor = this.row() + dRow;
        int colCoor = this.col() + dCol;
        if(rowCoor < 0 || colCoor < 0 || rowCoor >= numRows || colCoor >= numCols){
            return null;
        }
        return new Position(rowCoor, colCoor);
    }
